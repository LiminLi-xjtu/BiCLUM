...
def umap_plot_colors(adata, dataset_name, omic_colors, cell_type_colors, vis_dir):
    ...
    for i, method in enumerate(obsm_names):
        ...
        if dataset_name == 'BMCITE_s1d1_s1d2' or dataset_name == 'BMCITE_s1d2_s3d7':
            fig_right = sc.pl.umap(adata, size=100, color="cell_type2", palette=cell_type_colors, title='', ax=inner_ax2, show=False)
        else:
            fig_right = sc.pl.umap(adata, size=100, color="cell_type", palette=cell_type_colors, title='', ax=inner_ax2, show=False)

        if method == 'raw_legend':
            handles1, labels1 = inner_ax1.get_legend_handles_labels()
            handles2, labels2 = inner_ax2.get_legend_handles_labels()

            legend1 = inner_ax1.legend(handles1, labels1, loc='upper center', bbox_to_anchor=(0.5, -0.2),
                                       borderaxespad=0., ncol=1, fontsize=22, title='Omic Types',
                                       handletextpad=1.5, handlelength=2.5, labelspacing=1.5, markerscale=2.0)
            plt.setp(legend1.get_texts(), fontsize=22)
            plt.setp(legend1.get_title(), fontsize=23)
            legend1.get_frame().set_linewidth(0)

            legend2 = inner_ax2.legend(handles2, labels2, loc='upper center', bbox_to_anchor=(0.5, -0.2),
                                       borderaxespad=0., ncol=1, title='Cell Types',
                                       handletextpad=1.5, handlelength=2.5, labelspacing=1.5, markerscale=2.0)
            plt.setp(legend2.get_texts(), fontsize=22)
            plt.setp(legend2.get_title(), fontsize=23)
            legend2.get_frame().set_linewidth(0)

        else:
            inner_ax1.legend_.remove()
            inner_ax2.legend_.remove()
